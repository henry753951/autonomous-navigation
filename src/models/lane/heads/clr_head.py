...
from src.models.lane.losses.accuracy import accuracy
from src.models.lane.losses.focal_loss import FocalLoss
from src.models.lane.losses.lineiou_loss import liou_loss
from src.models.lane.ops import nms
from src.models.lane.registry import HEADS
from src.models.lane.utils.dynamic_assign import assign
from src.models.lane.utils.roi_gather import LinearModule, ROIGather
from src.models.lane.utils.seg_decoder import SegDecoder
...
class CLRHead(nn.Module):
    ...
    def generate_priors_from_embeddings(self):
        ...
        priors[:, 6:] = (
            ...
            + (
                (1 - self.prior_ys.repeat(self.num_priors, 1) - priors[:, 2].unsqueeze(1).clone().repeat(1, self.n_offsets))
                * self.img_h
    ...
    def predictions_to_pred(self, predictions):
        ...
        for lane in predictions:
            ...
            mask = ~((((lane_xs[:start] >= 0.0) & (lane_xs[:start] <= 1.0)).cpu().numpy()[::-1].cumprod()[::-1]).astype(bool))
            lane_xs[end + 1 :] = -2
    ...
    def loss(self, output, batch, cls_loss_weight=2.0, xyt_loss_weight=0.5, iou_loss_weight=2.0, seg_loss_weight=1.0):
        ...
        for stage in range(self.refine_layers):
            ...
            for predictions, target in zip(predictions_list, targets, strict=False):
